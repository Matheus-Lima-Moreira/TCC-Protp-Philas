...
if (isset($_POST["btnOk"])) {
  ...
  $Atendimento->descricao = $_POST["edtOutro"];
  // $Atendimento->tempo_previsto = "";
  $Atendimento->data_marcada = $_POST["edtData"];
  // $Atendimento->data_iniciada = "";
  // $Atendimento->data_finalizada = "";
  $Atendimento->cod_motivo = $_POST["slcMotivo"];
  $Atendimento->cod_atendido = $_SESSION["id"];
  $Atendimento->cod_atendente = $_POST["edtAtendente"];

  $Atendimento->criar($Atendimento);
}
?>
        ...
        <div>
          <select name="slcMotivo" id="slcMotivo">
            <option value="" disabled selected>Motivo</option>
              ...
              $sql = "SELECT * FROM `tb_motivo`";

              $result = (new Conexao())->consulta($sql);
