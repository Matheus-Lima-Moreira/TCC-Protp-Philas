      ...
      $input.datetimepicker({
        allowTimes: allowTimes.filter(function (value) {
          for (const range of data) {
            start = moment(range.inicio, 'HH:mm').subtract(tempo_previsto, 'm').format('HH:mm');
            end = range.termino;
...
// API URL
var urlAPI = window.location;
urlAPI = urlAPI.protocol + urlAPI.host + '/TCC/Protp_Philas/mvc' + '/api/v1';
  ...
  $(function () {
    motivo.on('change', function () {
      ...
      if (selected.val() == '') {
        ...
        tempo_previsto = selected.data('tempo_previsto');
      }
    });
  });
